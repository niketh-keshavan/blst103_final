"""
HOLC Redlining – Combined Box Plots for Atlanta
=================================================
...
def load_holc_data():
    ...
    if not os.path.exists(cache_path):
        raise RuntimeError(f"HOLC GeoJSON not found at {cache_path}. Run holc_k8_education.py first.")
...
for i, g in enumerate(grades):
    ...
    pos_c = center - width / 2 - 0.02
    pos_h = center + width / 2 + 0.02
    positions_college.append(pos_c)
    positions_hs.append(pos_h)
...
for i, g in enumerate(grades):
    ...
    if len(subset) == 0:
        continue
    center = i * (2 * width + gap)
    pos_i = center - width / 2 - 0.02
    pos_p = center + width / 2 + 0.02
    positions_income.append(pos_i)
    positions_poverty.append(pos_p)
...
print("\nDone! Both combo box plots saved.")
