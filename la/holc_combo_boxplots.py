"""
HOLC Redlining – Combined Box Plots for Los Angeles
=====================================================
...
def download_life_expectancy():
    """Download life expectancy from CDC USALEEP for LA County."""
    print("  Querying CDC USALEEP data for Los Angeles County...")
    url = (
        "https://data.cdc.gov/resource/5h56-n989.json"
        "?$limit=5000"
        "&state_name=California"
        f"&county_name={requests.utils.quote('Los Angeles County, CA')}"
    )
    try:
        resp = requests.get(url, timeout=30)
        if resp.status_code == 200:
            records = resp.json()
            print(f"    Los Angeles County: {len(records)} tracts")
        else:
            raise RuntimeError(f"HTTP {resp.status_code}")
    except Exception as e:
        raise RuntimeError(f"Could not download USALEEP data: {e}")

    df = pd.DataFrame(records)
    df["life_expectancy"] = pd.to_numeric(df["le"], errors="coerce")
    df["tract_code"] = (
        df["full_ct_num"].astype(str)
        .str.replace(".", "", regex=False)
        .str.zfill(6)
    )
    df["GEOID_2010"] = STATE_FIPS + "037" + df["tract_code"]
    df = df.dropna(subset=["life_expectancy"])
    df = df[df["life_expectancy"] > 0]
    return df


def compute_dominant_grade(holc, tracts):
...
# Merge life expectancy (2010 GEOIDs → 2020 GEOIDs best-effort)
tracts_le = tracts.merge(
    le_data[["GEOID_2010", "life_expectancy"]],
    left_on="GEOID", right_on="GEOID_2010", how="left"
).dropna(subset=["life_expectancy"])

if len(tracts_le) < 5:
    print("  Few exact matches — trying prefix match...")
    le_data["GEOID_prefix"] = le_data["GEOID_2010"].str[:9]
    tracts_tmp = tracts.copy()
    tracts_tmp["GEOID_prefix"] = tracts_tmp["GEOID"].str[:9]
    le_prefix = le_data.groupby("GEOID_prefix")["life_expectancy"].mean().reset_index()
    tracts_le = tracts_tmp.merge(le_prefix, on="GEOID_prefix", how="inner")

print("  Computing HOLC grade overlays...")
grade_edu = compute_dominant_grade(holc, tracts_edu)
grade_inc = compute_dominant_grade(holc, tracts_inc)
grade_le = compute_dominant_grade(holc, tracts_le)
...
le_analysis = tracts_le.merge(grade_le, on="GEOID", how="inner").dropna(subset=["life_expectancy"])

print(f"  Education analysis: {len(edu_analysis)} tracts")
print(f"  Income analysis:    {len(inc_analysis)} tracts")
print(f"  Life expectancy:    {len(le_analysis)} tracts")
...
ax3.set_title("Life Expectancy by HOLC Redlining Grade — Los Angeles",
              fontsize=15, fontweight="bold", pad=15)
